...
@Component({
  selector: 'app-receiving-statistics',
  standalone: true,
  imports: [MiniTableComponent, MatCardModule, ReactiveFormsModule],
  templateUrl: './receiving-statistics.component.html',
  styleUrl: './receiving-statistics.component.scss',
})
export class ReceivingStatisticsComponent {
  displayedColumns = [
    'number',
    'asnNumber',
    'commodityCode',
    ...
    if (newData) {
      const lastNum =
        this.stockManagmentData.length > 0
          ? Math.max(
              ...this.stockManagmentData.map(
                (item: { num: string | number }) => +item.num
              )
            )
          : 0;
  ...
  saveData(): void {
    localStorage.setItem(
      'stockManagmentData',
      JSON.stringify(this.stockManagmentData)
    );
  }
  ...
  updatedRow(selectedRow: any, updatedData: any) {
    const index = this.stockManagmentData.findIndex(
